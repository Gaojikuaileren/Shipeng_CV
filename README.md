...
**排除你自己的访问**：在控制台敲 `/paichu=1`（`/paichu=0` 取消），底部会显示本机排除了没有。
手机等别的设备上打不开控制台时，用 `你的域名/?v=ue&me=1` 也一样 —— 参数当场从地址栏抹掉。
两种方式写的是同一个本地标记，从此这个浏览器永不打点（访问、PDF、扫码、停留全跳过）。
`mode=full` 也一并视为本人。
每台设备都要单独设一次；换浏览器 / 清数据 / 无痕窗口要重设 —— 这是不用 cookie 的必然结果。
  · `/shelp` 列出全部指令（表从 `ROUTES` / `CLEAN` 现算，加变体自动多两行，不会写着一条不存在的）
  · `/paichu=1` 把本机排除在统计之外，`/paichu=0` 取消
